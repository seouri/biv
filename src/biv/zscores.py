...
def calculate_growth_metrics(
    ...
    # Unit warnings with statistical tests
    ...
    if "_bivbmi" in measures and "mod_bmiz" in result:
        # BMI: mod_z < -4 or >8
        biv_bmi_flag = (result["mod_bmiz"] < -4.0) | (result["mod_bmiz"] > 8.0)
        biv_bmi_flag = np.where(np.isnan(result["mod_bmiz"]), False, biv_bmi_flag)
