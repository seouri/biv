...
def render_outlier_controls(
    ...
    """
    Render controls for marking/unmarking outliers and adding comments.
    ...
    with col2:
        if st.button(
            "💾 Save Comment", 
            width="stretch",
            help="Save the comment for this measurement"
        ):
            set_point_comment(selected_index, comment)
...
def _save_current_state(
    ...
    patient_id : str
        Patient identifier
    error_indices : Set[int]
        Set of outlier indices (optional, will use session state if not accurate)
    is_complete : bool
